...
import { ArrowLeft, Sparkles, Layers, Shield, Zap, Award, Flame, Star, BookOpen, Home } from 'lucide-react';
...
export default function CardDetailPage() {
  ...
  const relatedCards = useMemo(() => {
    ...
    return Array.from(grouped.entries()).slice(0, 4).map(([coreName, cards]) => ({
      coreName,
      cards,
  ...
  return (
    <main className="relative min-h-screen bg-[#0a0c14] text-white overflow-x-hidden overflow-y-auto custom-scrollbar selection:bg-[#e6007e]/30">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        {/* Navigation Bar - Clear Separation Between Gallery & Home */}
        <div className="flex items-center justify-between gap-4 mb-8 flex-wrap">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Link
              href="/?gallery=open"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-[#e6007e] to-[#4694d1] text-white font-extrabold text-sm transition-all shadow-lg shadow-pink-500/25 border border-white/30 hover:scale-105 hover:brightness-110 active:scale-95"
            >
              <Layers size={17} />
            ...
            <Link
              href="/"
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/15 text-white/90 font-bold text-sm transition-all shadow-md hover:scale-105 active:scale-95"
            >
              <Home size={17} />
          ...
          <div className="flex items-center gap-2">
            <span className="px-3.5 py-1 rounded-full text-xs font-black uppercase tracking-wider bg-[#e6007e]/20 text-[#ff4081] border border-[#e6007e]/40">
              {cardGroup?.categoryLabel || 'CHIBI TIÊN CẢNH'}
            </span>
            <span className="px-3.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-white/10 text-[#87dff6] border border-white/15">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-start">
            ...
            <div className="lg:col-span-5 flex flex-col items-center">
              <motion.div
                ...
                className="w-full max-w-[360px] sm:max-w-[400px] aspect-[245/342.6] rounded-[22px] overflow-hidden relative cursor-grab shadow-[0_30px_90px_rgba(0,0,0,0.85),0_0_60px_rgba(70,148,209,0.4)] border-2 border-white/30 group"
                style={{
              ...
              >
                ...
                {/* Holographic light reflection sheen - only on hover */}
                <div
              ...
              {cardGroup.cards.length > 1 && (
                <div className="mt-6 flex flex-wrap items-center justify-center gap-2 p-2 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md">
                  <span className="text-xs font-bold text-white/50 px-2 uppercase">Phiên bản:</span>
            ...
            <div className="lg:col-span-7 flex flex-col gap-6">
              {/* Header Info */}
              <div className="p-6 sm:p-8 rounded-3xl bg-white/[0.04] border border-white/10 backdrop-blur-xl shadow-2xl">
                <div className="flex items-center gap-3 mb-3 flex-wrap">
                  <span className="px-3.5 py-1 rounded-full text-xs font-black uppercase tracking-wider bg-gradient-to-r from-[#e6007e] to-[#ff4081] text-white shadow-md">
                    {cardGroup.faction}
                  </span>
                ...
                {currentCard.title && (
                  <p className="mt-2 text-base font-bold text-[#80c6ff] tracking-wide">
                    {currentCard.title}
                  </p>
                )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div className="p-4 rounded-2xl bg-white/[0.04] border border-white/10 backdrop-blur-md flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-cyan-500/20 text-[#87dff6] flex items-center justify-center">
                    <Sparkles size={20} />
                  </div>
                  <div>
                    <div className="text-[11px] font-bold text-white/50 uppercase">Độ hiếm</div>
                    <div className="text-sm font-black text-white">{currentCard.rarity || 'Chí Tôn Thần Thoại'}</div>
                  </div>
                </div>

                <div className="p-4 rounded-2xl bg-white/[0.04] border border-white/10 backdrop-blur-md flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-pink-500/20 text-[#ff4081] flex items-center justify-center">
                    <Flame size={20} />
                  </div>
                  <div>
                    <div className="text-[11px] font-bold text-white/50 uppercase">Công Pháp / Nguyên Tố</div>
                    <div className="text-sm font-black text-white">{currentCard.element || 'Thái Cực Âm Dương'}</div>
                  </div>
                </div>

                <div className="p-4 rounded-2xl bg-white/[0.04] border border-white/10 backdrop-blur-md flex items-center gap-3 col-span-2 sm:col-span-1">
                  <div className="w-10 h-10 rounded-xl bg-blue-500/20 text-[#80c6ff] flex items-center justify-center">
                    <Shield size={20} />
                  </div>
                  <div>
                    <div className="text-[11px] font-bold text-white/50 uppercase">Môn Phái / Thế Lực</div>
                    <div className="text-sm font-black text-white truncate max-w-[120px]">{cardGroup.faction}</div>
                  </div>
                </div>
              </div>

              {/* Related Cards in Same Category */}
              {relatedCards.length > 0 && (
                <div className="p-6 rounded-3xl bg-white/[0.03] border border-white/10 backdrop-blur-md">
                  <h3 className="text-base font-extrabold uppercase tracking-wider text-white/90 mb-4 flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {relatedCards.map((rel, idx) => (
                      <Link
                        key={idx}
                        href={`/cards/${encodeURIComponent(rel.coreName)}`}
